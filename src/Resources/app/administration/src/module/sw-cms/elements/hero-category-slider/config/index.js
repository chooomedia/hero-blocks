...
export default {
    ...
    computed: {
        ...
        imageCountOptions() {
            ...
            return [
                ...
                {
                    value: '3',
                    label: this.$tc('sw-cms.elements.categorySlider.config.imageCount.options.three'),
                },
                {
                    value: '4',
                    label: this.$tc('sw-cms.elements.categorySlider.config.imageCount.options.four'),
                },
            ];
        },
    ...
    methods: {
        createdComponent() {
            ...
            // WICHTIG: Performance-optimiert - mehr Kategorien laden (100 statt 25)
            // sw-entity-multi-select hat integrierte Suche, lädt bei Bedarf mehr
            // WICHTIG: Erlaube Standard-Kategorien (page) und Landing Page Kategorien (landing_page)
            // WICHTIG: translations association für sw-entity-multi-select
            // WICHTIG: media association für Dropdown-Bilder
            // WICHTIG: ALLE Kategorien anzeigen (ohne mediaId Filter)
            if (!this.categoryCriteria) {
                this.categoryCriteria = new Criteria(1, 100); // Performance: 100 Kategorien initial (statt 25)
                ...
                // WICHTIG: KEIN Filter nach mediaId - ALLE Kategorien anzeigen
                // Kategorien ohne Bild werden im Frontend übersprungen (siehe TypeDataResolver)
                ...
                console.log('[CategorySlider Config] Criteria created (limit: 100, no mediaId filter):', this.categoryCriteria);
            }
            
            // Category Collection initialisieren (immer EntityCollection)
            // WICHTIG: Verwende createCollection() statt create() für leere EntityCollection
            if (!this.categoryCollection) {
                const context = Shopware.Context.api;
                this.categoryCollection = new Shopware.Data.EntityCollection(
                    '/category',
                ...
                console.log('[CategorySlider Config] Empty EntityCollection created:', this.categoryCollection);
            }
