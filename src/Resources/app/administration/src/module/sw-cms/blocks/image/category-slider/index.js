...
Shopware.Component.register('sw-cms-preview-category-slider', () => import('./preview/index.js'));
Shopware.Component.register('sw-cms-block-category-slider', () => import('./component/index.js'));
Shopware.Component.register('sw-cms-block-config-category-slider', () => import('./config/index.js'));
...
const blockConfig = {
    ...
    configComponent: 'sw-cms-block-config-category-slider',
    defaultConfig: {
    ...
    slots: {
        categoryImages: {
            type: 'category-slider', // WICHTIG: Eigenes Category Slider Element (nicht image-gallery)
            default: {
                // WICHTIG: Default-Config wird aus Element-Registrierung geladen (siehe elements/category-slider/index.js)
...
Shopware.Service('systemConfigApiService').getValues('HeroBlocks.config').then((values) => {
    if (values && values[configKey] !== undefined) {
        ...
        if (blockRegistry['category-slider']) {
            blockRegistry['category-slider'].hidden = !enabled;
        }
    }
}).catch(() => {
